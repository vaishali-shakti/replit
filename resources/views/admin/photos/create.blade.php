...
@section('content')
    <div class="container px-0">
        <h2 class="text-21 fw-bold mb-4">Create Photos</h2>
        <div class="profile px-0 pt-0">
            <div class="content-box">
                <form id="create_photos_form" method="post" action="{{ route('photos.store') }}"
    ...
    <script>
        $(document).ready(function() {
            $.validator.addMethod('filesize', function(value, element, param) {
                console.log(element.files[0].size,"filesize");
                return this.optional(element) || (element.files[0].size <= param);
            }, 'File size must be less than 2 MB');
            $('#create_photos_form').validate({
                rules: {
                    image: {
                        required: true,
