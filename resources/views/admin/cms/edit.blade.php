...
@section('content')
    <div class="container px-0">
        <h2 class="text-21 fw-bold mb-4">Edit CMS</h2>
        <div class="profile px-0 pt-0">
            <div class="content-box">
                <form id="edit_cms_form" method="post" action="{{ route('cms.update', $cms->id) }}" enctype="multipart/form-data">
                    @csrf
                    @method('PUT') <!-- This ensures the form sends a PUT request to update -->
                    <div class="row porfile-form pt-4">
                         <!-- Title -->
                         @if ($cms->slug_name == 'about-us')
                         <div class="form-group col-lg-12 col-md-12 mb-4">
                            ...
                            <input type="text" placeholder="Title" class="w-100 text-16 textdark px-4 py-3" name="title" maxlength="50" id="title" value="{{ old('title', $cms->title) }}" readonly>
                        </div>
                         @else
                         <div class="form-group col-lg-12 col-md-12 mb-4">
                            <label for="title" class="d-block text-18 fw-bold pb-2">Title<span class="text-danger">*</span></label>
                            <input type="text" placeholder="Title" class="w-100 text-16 textdark px-4 py-3" name="title" maxlength="50" id="title" value="{{ old('title', $cms->title) }}" required>
                            @error('title')
                        ...
                        @endif
                         ...
                         <div class="form-group col-lg-12 col-md-12 mb-4">
                            ...
                            <textarea name="description"  id="description" class="w-100 text-16 textdark px-4 py-3 textarea ckeditor" placeholder="Description" required>{{ old('description', $cms->description) }}</textarea>
                            @error('description')
                                <p class="text-danger">{{ $message }}</p>
                            @enderror
                        </div>
                         ...
                         <div class="form-group col-lg-12 col-md-12 mb-4">
                            ...
                            <input type="file" class="ss_help" name="image" id="image" accept="image/png,image/jpeg,image/jpg,image/webp">
                                @if ($cms ->image)
                                    ...
                                    <div class="mt-2">
                                        <img src="{{ $cms->image }}" id="image_preview_img" alt="Current Image" class="rounded-3" height="100px" width="100px"/>
                                    </div>
                                @else
            ...
            $('#edit_cms_form').validate({
                ...
                rules: {
                    title: {
                        ...
                        remote: {
                            type: 'get',
                            url: "{{ route('edit_title_unique_name') }}",
                            data: {
                                'title': function() {
                                    return $('#title').val();
                                },
